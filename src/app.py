        ...
        with st.expander(f"**{i+1}.** {q}", expanded=(i == 0)):
            if st.button(f"Get answer", key=f"onboard_{i}"):
                with st.spinner("Searching..."):
                    result = agent.invoke({"question": q, "role": role})
        ...
        if msg.get("citations"):
            with st.expander("📄 Sources"):
                for c in msg["citations"]:
                    st.markdown(f'<div class="citation-box"><b>{c["source"]}</b><br>{c["snippet"][:250]}...</div>', unsafe_allow_html=True)
...
if prompt:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Routing query → retrieving → generating..."):
            ...
            result = agent.invoke({"question": prompt, "history": history, "role": role})
